...
Shuffle money-jars. This project is [ledger-axon](https://github.com/jeroenvanmaanen/ledger-axon), but in [Rust](https://rust-lang.org/) (using [Dioxus](https://dioxuslabs.com/)). Like `ledger-axon`, `shuffle` is a ledger for multiple bank accounts loosely arranged according to the [six-jars](http://6jars.com/) principle.

For now the remainder of the `README` is autogenerated by Dioxus' `dx new`. It will be replaced by `shuffle`-specific documentation if I feel like it.
